...
for (prefix in company_prefixes) {
  ...
  # 7. Compute next-block return and lagged embeddings
  sub <- df %>%
    arrange(Date, Interval_4h) %>%
    mutate(
      return_pct = (.data[[close_col]] - .data[[open_col]]) / .data[[open_col]] * 100,
      across(all_of(embed_cols), ~lag(.x, 1, default = 0), .names = "{.col}_lag1")
    )

  # Define lagged embedding column names
  lag_embed_cols <- paste0(embed_cols, "_lag1")

  # 8. Filter only for available target and lagged features
  model_df <- sub %>%
    filter(!is.na(return_pct))
  total_rows <- nrow(model_df)
  if (total_rows == 0) next

  # 9. Build feature matrix and response (include current & lagged embeddings)
  ...
  y <- model_df$return_pct

  # 10. Fit LASSO with 5-fold CV
  set.seed(123)
  cvfit <- cv.glmnet(X, y, alpha = 1, nfolds = 5)

  # 11. Extract best lambda and nonzero features
  ...
  # 12. Plot CV curve and save
  png(paste0(prefix, "_lasso_cv.png"), width = 600, height = 400)
  plot(cvfit)
  dev.off()

  # 13. Bar plot of nonzero coefficients and save
  ...
  p_coef <- ggplot(coef_df, aes(x = reorder(Feature, Coefficient), y = Coefficient)) +
    ...
    theme_minimal()
  ggsave(paste0(prefix, "_coeffs.png"), plot = p_coef, width = 6, height = 4)
...
# 16. Plot lambda comparison and save
if (nrow(results) > 0) {
